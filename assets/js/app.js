import { readFileAsText, downloadFile, setupFileDropZone } from './modules/file-utils.js';
import { showAlert, clearAlert } from './modules/ui-utils.js';
import { convertRinToLx, serializeCSV, CSV_HEADER } from './modules/rin-converter.js';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function initRinConverter() {
        const rinFileInput = document.getElementById('rin-json-file');
        const rinBtn = document.getElementById('convert-json-btn');
        const rinAlertContainer = document.getElementById('rin-alert-container');

        setupFileDropZone('rin-drop-zone', 'rin-json-file', 'rin-file-name', (file) => {
        ...
        rinBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                downloadFile(`lxnet_export_${date}.csv`, csvString, 'text/csv;charset=utf-8');
                
                showAlert(rinAlertContainer, `转换成功！已生成 ${csvData.length} 条记录。`, 'success');
            } catch (err) {
                console.error(err);
    ...
    function initCsvMerger() {
        ...
        setupFileDropZone('csv1-drop-zone', 'csv-file-1', 'csv1-file-name', (file) => {
            selectedCsvFile1 = file;
            checkMergeButton();
            clearAlert(mergeAlertContainer);
        });
        
        setupFileDropZone('csv2-drop-zone', 'csv-file-2', 'csv2-file-name', (file) => {
        ...
        mergeBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                if (header1 !== header2) {
                    throw new Error(`文件表头不匹配！<br>文件1: ${header1}<br>文件2: ${header2}`);
                }
                ...
                const date = new Date().toISOString().split('T')[0];
                downloadFile(`merged_data_${date}.csv`, csvString, 'text/csv;charset=utf-8');

                showAlert(mergeAlertContainer, `合并成功！<br>文件1: ${csv1.data.length} 条记录<br>文件2: ${csv2.data.length} 条记录<br>总计: ${mergedData.length} 条记录`, 'success');
            } catch (err) {
                console.error(err);
    ...
    function initScoreEditorPage() {
        const editFileInput = document.getElementById('edit-json-file');
        const editAlertContainer = document.getElementById('edit-alert-container');
        const form = document.getElementById('edit-form');

        setupFileDropZone('edit-drop-zone', 'edit-json-file', 'edit-file-name', async (file) => {
            ...
            try {
                ...
                showAlert(editAlertContainer, "档案加载成功，可以开始编辑。", 'success');
            } catch (err) {
                console.error(err);
