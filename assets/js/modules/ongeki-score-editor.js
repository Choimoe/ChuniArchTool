...
export function initOngekiScoreEditor(jsonData) {
  ...
  // Editable inputs
  const inpPlatinumCritical = document.getElementById(
    "ongeki-edit-platinum-critical"
  );
  const inpCriticalBreak = document.getElementById(
    "ongeki-edit-critical-break"
  );
  ...
  const advancedJsonView = document.getElementById("ongeki-advanced-json-view");
  const jsonOriginalPre = document.getElementById("ongeki-json-original");
  const jsonModifiedViewPre = document.getElementById(
    "ongeki-json-modified-view"
  );
  const jsonModifiedEdit = document.getElementById("ongeki-json-modified-edit");
  ...
  let originalSnapshot = null; // 原始记录快照用于 diff
  ...
  songIdInput.addEventListener("change", () => {
    ...
    records.forEach((r) => {
      ...
      opt.textContent = `${date} - ${levelName} - Tech:${
        r.techScore || r.score || 0
      }`;
  ...
  recordList.addEventListener("change", () => {
    ...
    selectedRecord =
      (jsonData.userPlaylogList || []).find((r) => r.userPlayDate === date) ||
      null;
  ...
  [inpBell].forEach((inp) => {
    inp.addEventListener("input", (e) => {
      lastChangedInput = inp;
      if (inp === inpBell) {
        const bellVal = parseInt(inpBell.value) || 0;
        const totalBellVal = parseInt(inpTotalBell.value) || 0;
        if (bellVal > totalBellVal) {
          inpBell.value = totalBellVal;
        }
      }
      updateAll();
    });
  ...
  btnAdvanced.addEventListener("click", () => {
    ...
    if (advancedMode) {
      ...
      if (selectedRecord)
        originalSnapshot = JSON.parse(JSON.stringify(selectedRecord));
      if (jsonModifiedEdit) delete jsonModifiedEdit.dataset.userEdited;
      if (advancedJsonView) advancedJsonView.classList.remove("hidden");
    } else {
      ...
      if (advancedJsonView) advancedJsonView.classList.add("hidden");
    }
    updateAll();
  });
  ...
  function computeTechScore(cb, br, hit, miss, bell, totalBell, damage) {
    ...
    const bellScore =
      totalBell > 0 ? Math.floor((60000 * bell) / totalBell) : 0;
  ...
  function updateAll() {
    ...
    const { score: platinumScore, theory: platinumTheory } =
      computePlatinumScore(P, C, bell, totalBell, damage, nonBellNotes);
    ...
    const idConst = computeIdentificationConstant(
      techScore,
      isFullBell,
      isFullCombo,
      isAllBreak
    ...
    const totalConst =
      techConstRaw === null
        ? idConst.toFixed(3)
        : (parseFloat(techConstRaw) + idConst).toFixed(3);
    ...
    const statusStr = isAllBreak
      ? techScore === TECH_THEORY
        ? "AB+"
        : "AB"
      : isFullCombo
      ? "FC"
    ...
    if (advancedMode)
      renderJsonDiff({
        judgeCriticalBreak: C,
        judgeBreak: br,
        judgeHit: hit,
        judgeMiss: miss,
        bellCount: bell,
        totalBellCount: totalBell,
        damageCount: damage,
        techScore: techScore,
        techScoreRank: mapTechRank(techScore),
        platinumScore: platinumScore,
        platinumScoreStar: star,
  ...
  downloadBtn.addEventListener("click", () => {
    ...
    const damage = num(inpDamage);
    const nonBellNotes =
      fixedTotalNotes !== null ? fixedTotalNotes : C + br + hit + miss;
    const techScore = computeTechScore(
      C,
      br,
      hit,
    ...
    modified.techScoreRank =
      rankCodeMap[techRankName] || modified.techScoreRank;
    ...
    modified._calcIdentificationConstant = computeIdentificationConstant(
      techScore,
      isFullBell,
      isFullCombo,
      isAllBreak
    );
    modified._calcTotalConstant =
      modified._calcTechnicalConstant === null
        ? modified._calcIdentificationConstant
        : parseFloat(modified._calcTechnicalConstant) +
          modified._calcIdentificationConstant;

    const idx = (jsonData.userPlaylogList || []).findIndex(
      (l) => l.userPlayDate === selectedRecord.userPlayDate
    );
    if (idx !== -1) jsonData.userPlaylogList[idx] = modified;

    if (Array.isArray(jsonData.userMusicDetailList)) {
      const detail = jsonData.userMusicDetailList.find(
        (d) => d.musicId === modified.musicId && d.level === modified.level
      );
      if (detail) {
        if (
          typeof detail.techScoreMax !== "number" ||
          techScore > detail.techScoreMax
        ) {
        ...
        if (
          typeof detail.platinumScoreMax !== "number" ||
          platinumScore > detail.platinumScoreMax
        ) {
        ...
        if (isAllBreak) {
          if ("isAllBreak" in detail && !detail.isAllBreak)
            detail.isAllBreak = true;
          if ("isAllBreake" in detail && !detail.isAllBreake)
            detail.isAllBreake = true;
        }
      }
    }

    // 导出后更新原始快照（视为新基线）并刷新 diff
    if (advancedMode) {
      // 用户编辑的 JSON（可编辑文本框）优先覆盖 modified 对应字段
      if (jsonModifiedEdit && jsonModifiedEdit.value.trim()) {
        try {
          const userObj = JSON.parse(jsonModifiedEdit.value);
          // 仅覆盖原始快照中出现的键，避免引入未知结构
          Object.keys(originalSnapshot || {}).forEach((k) => {
            if (k in userObj) modified[k] = userObj[k];
          });
        } catch (e) {
          // 解析失败忽略，不阻塞导出
        }
      }
      originalSnapshot = JSON.parse(JSON.stringify(modified));
      if (jsonModifiedEdit) delete jsonModifiedEdit.dataset.userEdited;
      renderJsonDiff();
    }
  ...
  function renderJsonDiff(simulatedOverride) {
    if (
      !advancedJsonView ||
      !jsonOriginalPre ||
      !jsonModifiedViewPre ||
      ...
      return;
    jsonOriginalPre.textContent = JSON.stringify(originalSnapshot, null, 2);
    const orig = originalSnapshot;
    const sim = {};
    Object.keys(orig).forEach((k) => {
      if (simulatedOverride && k in simulatedOverride)
        sim[k] = simulatedOverride[k];
      else sim[k] = orig[k];
    });
    ...
    keys.forEach((k, idx) => {
      ...
      const changed = JSON.stringify(val) !== JSON.stringify(origVal);
      const valStr = JSON.stringify(val);
      const comma = idx === keys.length - 1 ? "" : ",";
      const keyPart = changed
        ? `<span class="diff-changed">"${k}"</span>`
        : `"${k}"`;
      lines.push(`  ${keyPart}: ${valStr}${comma}`);
    });
    const highlighted = `{\n${lines.join("\n")}\n}`;
    jsonModifiedViewPre.innerHTML = highlighted.replace(/<span class="diff-changed">([^"]+)<\/span>/g, function(m, p1) {
      return '<span class="diff-changed">' + p1 + '</span>';
    });
    // 若用户尚未手动编辑，则同步更新可编辑文本框
    if (!jsonModifiedEdit.dataset.userEdited) {
      jsonModifiedEdit.value = JSON.stringify(sim, null, 2);
    }
  }

  if (jsonModifiedEdit) {
    jsonModifiedEdit.addEventListener("input", () => {
      jsonModifiedEdit.dataset.userEdited = "1";
    });
  }
}
